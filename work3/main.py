...
def draw_curve_kernel(n: ti.i32):
    ...
    for i in range(n):
        ...
        if 0 <= x_pixel < WIDTH and 0 <= y_pixel < HEIGHT:
            # 浅蓝色
            pixels[x_pixel, y_pixel] = ti.Vector([0.2, 0.7, 1.0])
...
def main():
    ...
    while window.running:
        ...
        if current_count > 0:
            ...
            # 控制点：灰色
            canvas.circles(gui_points, radius=0.006, color=(0.6, 0.6, 0.6))
            ...
            if current_count >= 2:
                ...
                # 连线：白色
                canvas.lines(gui_points, width=0.002, indices=gui_indices, color=(1.0, 1.0, 1.0))
